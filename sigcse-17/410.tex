...
the Simics system emulator \cite{simics} (described below).
...
\x{cond_wait()} and \x{deschedule()} (the system call
...
% TODO: Figure out how to left-justify these bullet poince.
\begin{itemize}
	...
	\item Because concurrency bugs are triggered rarely,
		finding them requires code capable of running
	...
	\item Robust code often requires a different structure
		...
		cannot do so if kernel memory has been exhausted.
\end{itemize}
