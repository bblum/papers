...
we consider a data race a bug only when it results in a visible failure state (e.g., crash or deadlock).
(From a C++ perspective, we assume all concurrent accesses are implemented by {\tt std::atomic}.)
