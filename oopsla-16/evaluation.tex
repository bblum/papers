...
% TODO: Where should this paragraph be?
{\bf Variance.}
Because \quicksand~can be nondeterministic in which state spaces get scheduled for testing more often,
we quantified the variance in the distribution of QS-DR's bugs with a permutation test.
We re-ran
% TODO: X=? 100?
%X\% of the tests in
the QS-DR experiment and compared the two observed distributions,
finding the observed area between the curves to be among the middle
% TODO
Y\%
% FIXME: How many randomly sampled permutations?
of possible areas between permuted distributions,
and furthermore finding the observed area to be
% TODO
Z\%
...
\begin{figure}[t]
	...
	\includegraphics[width=0.48\textwidth]{dowefindbugsfaster-squashed.pdf} \\
		(a) Bugs found by elapsed CPU time. A more direct comparison than (b),
...
This is unsurprising: the seed subsets of PPs (\sect{\ref{sec:initial-pp}}) QS-Sync-Only is limited to are much less flexible than ICB.
This result suggests that in future work, \quicksand~should consider using ICB in parallel with its initial PP configuration when it finds no data-race candidates to test.}
%\footnote{
...
the sheer number of accesses during each new execution (often thousands) added significant performance overhead to the MC when computing DPOR and backtracking\footnote{
	...
	\cite{portend}, but this sacrifices soundness, as shown in \sect{\ref{sec:eval-falseneg}}.}
}.
