...
\begin{itemize}
	...
	\item {\bf SSS-MC-ICB}: With PPs as above, but using ICB \cite{chess-icb} with BPOR \cite{bpor} instead of DPOR to find bugs faster, and
...
and ultimately conclude 10 CPU-hours with 151\% as many bugs in total.
...
% TODO: Incorporate the following point.
% "Before the break-even point at 45 seconds (or whatever), SSS-MC approaches seem to outperform \quicksand,
% but converting this CPU-time advantage into faster wall-clock performance is itself an open research problem \cite{parallel-dpor}.
