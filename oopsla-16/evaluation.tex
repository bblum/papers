...
\begin{itemize}
		\revisionx{
	\item {\bf QS-Limited-HB}: \quicksand~using Limited-HB to add data-race PPs,
...
\revisionx{Compared to SSS-MC-Shared-Mem (the best SSS-MC approach in the long term),} we ultimately concludes 10 CPU-hours with \revisionx{125\%} as many bugs in total \revisionx{when using Limited HB}.
...
% TODO CAMREADY cite simsa thesis
...
\begin{table*}[t]
	...
	\begin{tabular}{r|c||c|c|c|c|c|c||c|c||c|c}
		...
		% FIXME CAMREADY: update FRMs
					% TODO: update to be ShMem not DPOR
											% TODO: recount with ICB
				% #test  sssmc   icb      qs      dronly  nondets  FRMs   timeout comp.SSes DRPPs untested DRPPs
			% TODO: If possible, add a column for how many false negatives (incorrect verif claims) ShMem had.
	...
	\caption{Summary of bugs and data races found by each test program.
		% TODO CAMREADY: Add Shared-Mem
		DPOR/ICB are the controls; QS is \quicksand.
		``DR bugs'' counts among \quicksand's bugs how many required data-race PPs to expose (\sect{\ref{sec:eval-sssmc}});
	among those, ``Nondeterministic DR bugs'' counts how many candidates required MC integration to identify (\sect{\ref{sec:eval-dr}}).
	``Malloc DRs'' counts how many malloc-recycle false positives we suppressed (\sect{\ref{sec:eval-dr}}).
	% TODO CAMREADY: count timeouts for ICB instead?
	``Mutual timeouts'' counts how often both SSS-MC-DPOR and \quicksand~timed out with no bug found;
...
which is consistent with the results of \cite{chess-icb}.
...
\begin{table}[h]
	...
	\caption{\revisionx{Distribution of preemption bounds among bugs found by ICB \cite{chess-icb} control experiments. (Bound 0 means the bug was found by switching threads only on {\tt yield()}s.)}}
\end{table}
...
Among QS-Limited-HB's verified tests, 36 contained no data-race candidates whatsoever,
...
This trade-off is attributable to the fact that QS-Limited-HB need not wait to test many alternate thread interleavings before a potential data-race candidate is confirmed; rather, it can add new jobs to start testing potential races immediately.
