...
at least 165 of which will expose bugs under one or more MC trials. % QS bugs + bugs where SSSMC found but QS didnt
...
%To match this,
\quicksand~reports the CPU-time spent in addition to the wall-clock time for a direct comparison
...
In Figure~\ref{fig:dowefindbugsfaster}(b), we give \quicksand~full credit for its inherent parallelism,
and with a 10 core allocation, it outperforms SSS-MC for any fixed budget of wall-clock time.}
