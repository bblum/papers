...
\begin{itemize}
	...
	\item $\conflicts{t_\alpha}{t_\beta}$ indicates the set of shared memory conflict pairs between $t_\alpha$ and $t_\beta$.
		%memory access pairs between $t_\alpha$ and $t_\beta$ such that among each pair,
		%one access belongs to each thread $\alpha$ and $\beta$, the address is the same, and at least one is a write;
		%i.e., precisely the shared memory conflict relation required by DPOR to determine dependent interleavings.
\end{itemize}
...
\begin{definition}[Relevant preemption point]
	...
	%$\ppinstr{\mathcal{I}}{\alpha}{i}$ is a sync API boundary (including a voluntary yield),
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Otherwise, $\conflicts{\ppothers{\mathcal{I}}{\alpha}{i}}{t_{\alpha{}(i+1)}} = \emptyset$,
			...
			\]
			In other words, we reorder {\ppothers{\mathcal{I}}{\alpha}{i}} with the entire next transition by thread $\alpha$.
...
%interleave threads sufficiently to expose
...
\\
	...
	%interleaving threads around
	sync API PPs alone.
	Here, preempting during line 1's race is necessary to force the program
	%into the control flow which can race on line 5.
...
We proceed by induction on the PPs according to their order in $\mathcal{I}$%the order of their preemptions.
...
By the definition of relevance, %fully-relevant interleaving,
...
\begin{theorem}[Convergence of Iterative Deepening]
	% restatement of same theorem from beginning
	If a bug is exposed by an interleaving in the \naive state space, Iterative Deepening will eventually test an equivalent interleaving which exposes the same bug.
\end{theorem}
...
%\footnote{
%	In the paper we noted that when a bug is found, \quicksand~cancels all superset state spaces,
%	anticipating the same bug.
...
%	This also suffices for the sake of verification.}.
