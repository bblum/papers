...
\begin{eqnarray*}
	...
	&|& v \leftarrow \mathsf{thread\_fork} \quad | \quad \mathsf{thread\_exit} \\
	&|& \mathsf{local}
\end{eqnarray*}
...
a stack of value maps of thread-local variables ($v$),
...
and a set of mutex objects, denoted $m$.
...
although it assumes a sequentially-consistent memory model \cite{tsopso}.
\begin{eqnarray*}
	\mathcal{E} &::=& [\mathcal{T}] \times [a \rightarrow \mathsf{int}] \times [m \rightarrow \mathsf{bool}] \\
	\mathcal{T} &::=& [\mathcal{I}] \times [v \rightarrow \mathsf{int}] \times \mathsf{bool}
\end{eqnarray*}
...
$\mathsf{deschedule}$ and $\mathsf{make\_runnable}$ allow threads to manipulate their runnability,
...
{\bf Local state.}
$\mathsf{local}$ represents any thread-local instruction, such as flow control, function definition/calling, and modifying local variables.
