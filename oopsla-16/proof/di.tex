...
{\bf Local state.}
$\textit{local}$ represents any thread-local instruction, such as modifying local variables, flow control, function calling, and assertions. %, and side effects.
...
which occur immediately after any of $\mathcal{A}_{sync}$.
%$\mathsf{mutex\_lock}$,
...
All data-race PP predicates will occur immediately {\em before} a $\mathsf{read}$, $\mathsf{write}$, or atomic swap.
...
In the main paper, we used ``PP'' largely to denote what we now call ``PP predicates''.
