...
CHESS~\cite{chess} and dBug~\cite{dbug-ssv}, but is the first tool we know of to apply systematic exploration in a kernel development environment.
...
Suppose a student were testing thread death and reaping, and wanted decision points to appear in \x{wait} and \x{vanish} but not if they're also associated with virtual memory operations.
They should write \x{within_function wait vanish} and \x{without_function} \x{destroy_address_space}.
