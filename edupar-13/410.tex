...
The most straightforward way for an exiting task to
convey its exit status to its parent is for it to
\x{malloc()} an exit-status blob and enqueue it for
later processing by the parent.
However, this design suffers from a ``terminal irony'':
a program wishing to exit,
which will free up kernel memory,
