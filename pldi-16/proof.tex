...
We'll call the latter the {\em naive maximal state space}, and call the condition of testing all its interleavings {\em convergence}.
...
\begin{theorem}[Convergence of Iterative Deepening]
	If a bug is exposed by an interleaving in the naive maximal state space, Iterative Deepening will eventually test an interleaving which exposes the same bug.
	\label{thm:convergence}
\end{theorem}

First we will prove a slightly weaker statement, limiting the naive approach's PPs to data races,
then strengthen it by showing that the all interleavings in the naive maximal state space
are equivalent to some interleaving made of data-race PPs (and sync API PPs) alone.

\subsection{Saturation of data-race PPs}

\begin{lemma}
	If a bug is exposed by any interleaving comprising any combination of data-race PPs and sync API PPs,
...
\begin{lemma}
	For any interleaving in the naive maximal state space, there exists an equivalent interleaving which uses only data-race PPs and sync API PPs.
...
\begin{proof}
By Lemma~\ref{lem:equivalent}, the naive interleaving must be equivalent to some interleaving built of data-race PPs and sync API PPs.
