...
Let
\[
	\mathcal{I} = \{(t_\alpha, p_\alpha)_1, (t_\beta, p_\beta)_1, ... (t_\alpha, p_\alpha)_n, ...\}
\]
be an interleaving trace, where the element $(t_\alpha, p_\alpha)_i$ represents the $i$th transition $t$ of thread $\alpha$, and the associated preemption point $p$.
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item If $\conflicts{\ppothers{\pai}}{\ppnext{\pai}} \ne \emptyset$, or if $t_{\alpha{}i}$ contains a sync API boundary instruction that's not already a PP,
			...
			In English, we have simply reordered $\ppothers{\pai}$ to between $\pppfx{\rho}$ and $\rho$, removing the irrelevant $\pai$ and adding a new PP $\pai'$, which is relevant by the construction of $\rho$.
		...
		\item Otherwise, $\conflicts{\ppothers{\pai}}{\ppnext{\pai}} = \emptyset$, and no instructions among $\ppnext{\pai}$ are a sync API boundary.
			...
			\]
			In English, we have reordered {\ppothers{\pai}} with the entire next transition by thread $\alpha$.
			We know this must be possible for the same reasons as above, and again, the transitions are independent, so $\mathcal{I}' \equiv \mathcal{I}$.
					       %&$(t_{\alpha{}(i+1)}) \setminus \pppfx{\rho}, p_{\alpha{}(i+1)}),$ & $ ...\}$ \\
	\end{itemize}
	This constitutes an algorithm for inductively converting all irrelevant PPs to relevant ones.
\end{proof}
...
The main challenge of this proof is that some data-race candidates are nondeterministic to find;
...
% TODO: define the special way of ordering the pps
