...
We believe this test suite's size allows for a more statistically significant comparison among MC strategies,
...
We know of no approach to quantify the probability, when a state space times out,
that a bug would be exposed by an untested interleaving.
\quicksand~does not attempt any such quantitative partial verification guarantee,
but can at least report which subsets of PPs produced state spaces that could be completed.

The SSS-MC control experiment timed out in 10 hours on 77 tests.
For these, we counted the number of state spaces \quicksand~was able to complete.
In two cases, \quicksand~also failed to complete anything; beyond these,
between 1 and 508 state spaces could be completed, with a mean of 72.8 and median of 30.
These completed state spaces guarantee that, if the test case could expose a bug,
