...
\begin{figure}[t]
	...
	\caption{Some data-race candidates may not be identified during a single program execution.
		...
		\quicksand~found 189\% as many bugs compared to the approach of prior work \cite{portend}.
	}
...
The second series shows all the data-race bugs \quicksand~found,
which includes the former type as well as new bugs involving nondeterministic data-races.
In total, among \quicksand's 68 total data-race bugs, just 36 could be found with single-pass data-race candidates alone.
...
are not intended to discover new candidates under subsequent runs. %before discovering any initial candidates.
...
other replay-based data-race tools \cite{racefuzzer,portend} should be configured to use a similar feedback loop as Iterative Deepening (\sect{\ref{sec:related-dr}}).
