...
%\newcommand\tak{\ensuremath{t_{\alpha{}k}}}
...
Unfortunately, $\mathcal{I}'$ is not reachable, as $\ppothers\pai'$ still includes any data-race PPs from $\ppothers\pai$,
...
and ending with the next sync API PP, which we'll call $\pbk$.
It follows from sync API PPs being reachable by definition,
and from the inductive hypothesis (which covers the prefix indicated by ``$...$''),
