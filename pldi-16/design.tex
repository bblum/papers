...
For testing user-space code, we begin with the four subsets of locking API PPs that we showed in Figure~\ref{fig:id}:
$\{yield\}$,
$\{yield,lock\}$,
$\{yield,unlock\}$,
...
Furthermore, we do not always increase the number of PPs when we find a new data race.
