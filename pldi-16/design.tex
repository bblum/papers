...
We focus on dynamic data-race detection~\cite{hybriddatarace,tsan} as the mechanism for finding new preemption candidates.
...
\begin{algorithm}[t]
	...
	\If{$\forall j \in \AllJobs,$ \GetPPSet{$j$} $\neq \{\alpha\}$}{
		AddNewJob($\{\alpha\}$, HeuristicPriority($\alpha$))
	}
