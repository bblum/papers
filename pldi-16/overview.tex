...
\begin{tabular}{rll}
	...
	6 & \texttt{x = tmp + 1;} & \\
	7 & \texttt{\hilight{olivegreen}{yield();}} & \\
	8 & \texttt{\hilight{commentblue}{mutex\_unlock(\&mx);}} & \\
	9 & \texttt{assert(x >= 1);}  & \\
	10 & & \texttt{assert(x >= 2);} \\
...
\end{tabular}
\caption{Example program with a data-race bug. In this interleaving, the assertion on line 10 will fail. Two data-race preemptions are required to expose the bug: one just before thread 1's line 6, and one just before thread 2's line 10.}
\label{fig:example}
\end{figure}
...
in our example program, these would be lines 1, 4, 7, and 8.
...
In Figure~\ref{fig:example}, lines 3 and 5 each race with 2, 6, and 9, and line 6 further races with 10.
