...
% TODO: Be proud of them instead of being dismissive.
...
Compared to us, \portend~can detect non-failing races which nevertheless cause suspiciously different program output,
while we depend on directly detecting failures.
It also uses symbolic execution to test input nondeterminism along with schedule nondeterminism,
while we explore the latter only.
However, \portend~does not perform MC-like exploration in advance of knowing data-race locations,
which we showed in \sect{\ref{sec:eval-falseneg}} can miss some bugs.
It is also limited to the POSIX synchronization API, so cannot verify lock implementations such as we do with \mxtest.
