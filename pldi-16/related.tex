...
SKI \cite{ski} approaches kernel code by choosing a random set of instructions in advance, %offsets from the start of the test,
...
although it requires code to be written in the PROMELA DSL.
...
Both SPIN and Inspect fix their set of PPs in advance, so could be combined with \quicksand~in future work.
%so cannot check implementations directly.

{\bf Other techniques.} Various improvements or alternatives to DPOR have been proposed, such as Dynamic Interface Reduction \cite{demeter}, Maximal Causality Reduction \cite{mcr}, DPOR for relaxed memory models \cite{tsopso}, and SAT-directed MC \cite{satcheck}.
