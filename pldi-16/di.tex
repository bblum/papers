...
The algorithm's purpose is to make decisions automatically about when to defer exploration of a state space,
...
and {\em single-state-space MC} (SSS-MC) (i.e., the approach of prior work).
...
This algorithm is the heart of Iterative Deepening; we list it in Figure~\ref{fig:shouldworkblock}.
The algorithm takes into account... TODO
Though it has worst-case performance of $O(mn)$ in the number of pending and blocked jobs,
in practice we find the non-constant portion runs very infrequently and is negligible compared to the exponentially-sized state spaces.
