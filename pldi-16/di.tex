...
Our method's main feature is that it understands that if \GetPPSet{$j_1$} $\subset$ \GetPPSet{$j_2$},
and $j_1$ is suspended, $j_2$'s state space is guaranteed to be strictly larger, so $j_2$ would take at least as long.
Hence we should avoid testing $j_2$ unless further time spent on $j_1$ reveals that it might finish in time after all.
%
...
when their ETAs are actually overestimates.
...
to avoid repeatedly resuming larger jobs very briefly while their ETAs only become worse
