...
so we also preempt just before {\tt cli} and just after {\tt sti}\footnote{
...
\quicksand~is configured to use the subsets
$\{ \}$,
$\{lock\}$,
$\{unlock\}$,
$\{cli\}$,
...
Other subsets are obviously possible, but as a heuristic, we don't bother with them.
