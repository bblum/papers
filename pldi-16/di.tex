...
We seed the exploration with the four subsets of ``hard-coded'' locking API PPs shown in Figure~\ref{fig:id}.
...
% FIXME: I'm really not satisfied with the flow of this paragraph.
Note that under a {\em pure} happens-before analysis,
these accesses are not concurrent at all
because of {\tt malloc}'s internal locking events,
and would not result in such false positives.
However, as \cite{hybriddatarace} and \cite{tsan} argue,
...
This makes it unnecessary to verify such data races by actively preempting threads during them,
...
We say WLOG that T1 performs the {\tt free} and T2 the subsequent {\tt malloc}; the other cases are similar.
...
The publish action must involved some thread communication, whether through a shared data structure or message-passing API.
