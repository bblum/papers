...
% TODO: find some way to point out that heuristic factor is needed because the estimates are imperfect
%(greater than some heuristic factor times the time remaining),
...
%Note that Algorithm~\ref{alg:shouldworkblock} accounts for the bizarre possibility that given two jobs,
%%$j_1,j_2$,
%\GetPPSet{$j_1$} $\subset$ \GetPPSet{$j_2$}
%but
%\GetETA{$j_1$} $>$ \GetETA{$j_2$}.
%This can arise because the estimation gets more accurate over time \cite{estimation},
%and one job perhaps ran much longer before suspending.
%% In such scenarios,
