...
\begin{theorem}
If a bug can be exposed by any thread interleaving possible by preempting on all instructions,
...
A data race candidate (and associated PPs) is reachable if it will be identified by an MC configured to preempt only on already-reachable PPs.
\end{definition}
...
We identify a particular class of false positive data-race candidates in which the memory was recycled by {\tt malloc} between the two accesses.
...
We now show that a PP will be identified during T1 between $p$ and $a_1$.
...
Note that we do not require PPs on the internal locking of {\tt malloc} or {\tt free},
which are ideal candidates to ignore via {\tt without\_function} (\sect{\ref{sec:landslide}}, \sect{\ref{sec:future}}) to reduce state space size.
